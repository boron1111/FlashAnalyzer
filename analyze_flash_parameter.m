...
for j=0:num-1
    parameter{j+1}=analyze_flash_parameter1(peak_intervals{j+1},interval_range(1,j+1),1);
%     
%     start_arr(j+1)=parameter{j+1}(15);
%     peak_arr(j+1)=parameter{j+1}(16);
%     end_arr(j+1)=parameter{j+1}(17);
%     disp([start_arr(j+1),peak_arr(j+1),end_arr(j+1)]);
end
% a=trace;
% plot(0:length(a)-1,a,'-',floor(start_arr),a(floor(start_arr)+1),'ro',floor(peak_arr),a(floor(peak_arr)+1),'g.',floor(end_arr),a(floor(end_arr)+1),'b*');

function parameter_new=analyze_flash_parameter1(flash_trace,t_offset,time_interval)
...
pvally=max(finding_vally(flash_trace(1:floor(tleft_temp)+1)),0);
pvally=max(pvally);
...
[vallyend,~]=finding_vally_swelling(downcurve(floor(vallyendhalf)+1:end),2,2);
if vallyend(1)<2
...
if (posv-t_half1)<0.5*(t_half1-tstart) && (peakv-basal)/basal>=0.1
    temp=flash_trace(fix((tstart+posv)/2)+1:fix(t_half1)+1);
...
if t_half1>3
    gradientup=convol_center_edgeTruncate(flash_trace(1:fix(t_half1)+1),[-1,1]);
...
k_left_m=(peakv-flash_trace(floor(tstartsp)+1))/(posv-tstartsp)/time_interval;
